...
class CronioWorker(object):
	...
	class CronioWorkerListener(stomp.ConnectionListener):
		...
		def on_message(self, headers, message):
			...
			try:				
				...
				if headers["destination"] == self.parent.CRONIO_WORKER_DEPENDENCY_OWN:
					self.parent.addDependencyMessage(message_obj, headers)
					...
					if message_obj["type"] == "operation" and "cmd" in message_obj:
						...
						elif cmd == "inform_dependency_worker":
	...
	def checkWorkersDependencies(self, cmd_id):
		checkResolvedMissing = session.query(CommandDeps).filter_by(to_run_cmd_id=cmd_id,resolved='No').first()
		if checkResolvedMissing:
			# Dependencies still have not been resolved.
			return False
		else:
			# Dependencies all have been resolved.
			checkResolvedAll = session.query(CommandDeps).filter_by(to_run_cmd_id=cmd_id,resolved='Yes',ok_to_run=False).all()
			getCommand =session.query(Commands).filter_by(cmd_id=cmd_id).first()
			api_log = getCommand["api_log"]
			if checkResolvedAll:
				# Send message to sender due to failed dependencies, will not run.
				...
				checkResolvedAll = session.query(CommandDeps).filter_by(to_run_cmd_id=cmd_id,resolved='Yes',ok_to_run=True).all()
				if checkResolvedAll:
	...
	def dependencyResolved(self, cmd_id, result_code, worker_id, sender_id):
		dependencyFound = session.query(CommandDeps).filter_by(needed_cmd_id=cmd_id,worker_id=worker_id,sender_id=sender_id).first()
		if dependencyFound:
			...
			session.commit()
			checkWorkersDependencies(dependencyFound.to_run_cmd_id)
			return True
		return False


	def runCMDFromDB(self,cmd_id):
		commandFoundDB = session.query(Commands).filter_by(cmd_id=cmd_id,status='pending').first()
		if commandFoundDB:
			message_obj = {"type": commandFoundDB["is_type"], "cmd": commandFoundDB["cmd"], "cmd_id": commandFoundDB["cmd_id"], "api_log": commandFoundDB["api_log"], "dependencies": commandFoundDB["dependencies"]}
	...
	def sendAPIInformWorkerDependencyResult(self, cmd_id, worker_id, result):
		data = {"cmd_id": cmd_id, "worker_id": self.CRONIO_WORKER_ID, "result": result}
	...
	def sendDependencyResultToWorker(self, message_obj, api_log):
		# CommandLog(cmd_id=cmd_id, cmd=json.dumps(cmd),is_type=is_type, sender=sender, dependencies=json.dumps(dependencies), result_code=10001, api_log=api_log)
		commandLogFound = session.query(CommandLog).filter_by(cmd_id=message_obj["cmd_id"]).first()
		if commandLogFound:
			self.sendAPIInformWorkerDependencyResult(message_obj["cmd_id"],message_obj["worker_id"], commandLogFound.result_code)
		else:
			# 1002 Could not be found
			self.sendAPIInformWorkerDependencyResult(message_obj["cmd_id"],message_obj["worker_id"], 10003)
			self.sendAPIInform({"dependency_data":message_obj, "result": 10003, "message": "Could not find CMD ID in Database.","message_at": str(datetime.datetime.now())},api_log)
